...
def carpeta_respaldos() -> Path:
    ...
    ruta_configurada = obtener("backup_carpeta_compartida")
    carpeta = Path(ruta_configurada) if ruta_configurada else settings.BACKUPS_DIR
...
def _respaldar_plantillas(sello: str) -> None:
    ...
    origen = settings.PLANTILLAS_DIR
...
def restaurar_respaldo(nombre_archivo: str) -> None:
    ...
    connections.close_all()  # cierra conexiones abiertas antes de tocar el archivo

    crear_respaldo()  # respaldo de seguridad del estado justo antes de restaurar

    for sufijo in ("", "-wal", "-shm"):
        candidato = Path(str(settings.DB_PATH) + sufijo)
        if candidato.exists():
            candidato.unlink()
    ...
    if carpeta_plantillas_respaldo.is_dir():
        destino_plantillas = settings.PLANTILLAS_DIR
        destino_plantillas.mkdir(parents=True, exist_ok=True)
